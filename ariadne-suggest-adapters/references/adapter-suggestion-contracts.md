# Ariadne Adapter Suggestion Contracts
...
`ariadne-suggest-adapters` plans adapter changes. It may write suggestion JSON/MD under `.codebase/meta/suggestions/`, but it must not directly modify agent entry files.
...
Do not create capability names like `ariadne-suggest-codex-adapter`.
...
```json
{
  ...
  "createdBy": "ariadne-suggest-adapters",
  "agentScope": ["codex"],
  "risk": "low",
  "sourceFingerprints": {

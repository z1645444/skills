# Pagepack Freshness Check Contracts
...
`pagepack-check-freshness` compares manifest source fingerprints with current source fingerprints.
...
```json
{
  ...
  "recommended": ["pagepack-suggest-refresh"]
}
```
...
`pagepack-check-freshness` diagnoses.

`pagepack-suggest-refresh` plans changes.

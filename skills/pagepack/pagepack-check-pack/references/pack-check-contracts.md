...
`pagepack-check-pack` checks `.codebase/` structure and machine-readable contracts only.
...
```text
error
  The pack is not usable by Pagepack until fixed.
...
`generatedBy` should be `pagepack-init` for newly bootstrapped packs. Existing packs that still record the legacy value `pagepack-suggest-init` are compatible v1 packs; report the legacy value as a warning or provenance note, not an error.

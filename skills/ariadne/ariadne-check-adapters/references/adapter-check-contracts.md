# Ariadne Adapter Check Contracts
...
`ariadne-check-adapters` checks whether agent entry files are thin Agent Adapters pointing to `.codebase/router.md`.
...
Do not create per-agent capability names such as `ariadne-check-codex-adapter`.
...
Use `ariadne-suggest-adapters --agent <name>` when the user checked a specific agent.
...
```json
{
  ...
  "recommended": ["ariadne-suggest-adapters --agent codex"]
}
```

# Ariadne Explain Contracts

This reference defines the v1 read-only traceability contract for Ariadne Explain. Human-facing output defaults to Simplified Chinese; preserve file paths, command names, API names, framework names, component names, identifiers, route names, and suggestion ids.

## Scope

`ariadne-explain` explains existing pack objects. It reads `.codebase/` state and reports traceability.
...
Do not recompute freshness. Recommend `ariadne-check-freshness` when needed.
...
Do not apply the suggestion. Recommend `ariadne-apply-suggestion <id>` when appropriate.
...
```json
{
  ...
  "ownership": null,
  "missingEvidence": [],
  "recommended": ["ariadne-apply-suggestion adapter-20260610-001"]
}
```

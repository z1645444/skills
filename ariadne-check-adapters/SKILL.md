---
name: ariadne-check-adapters
description: Read-only check for Ariadne Agent Adapters. Use when verifying whether Codex, Claude Code, Gemini CLI, Cursor, Antigravity, or another supported coding agent has an entry file that explicitly points to `.codebase/router.md` and instructs the agent to follow Task Router routes before coding, without generating adapter suggestions or modifying entry files.
---
...
5. Report status.
   ...
   - Recommend `ariadne-suggest-adapters` when a patch is needed.

...
完整参考见 [ariadne-usage.md](ariadne-usage.md)。
...
1. 在项目根目录运行 `ariadne-init`，直接创建初始 `.codebase/` Codebase Knowledge Pack。
2. 运行 `ariadne-check-pack` 检查结构是否完整。
3. 运行 `ariadne-suggest-adapters`，为当前 agent entry file 生成接入 `.codebase/router.md` 的建议。
4. review suggestion 后，用 `ariadne-apply-suggestion <suggestion-id>` 应用。

## 日常使用

- 改代码前让 agent 读取 `.codebase/router.md`，按任务类型加载所需 Runtime Docs。
- 项目或 framework 变化后，先 `ariadne-check-freshness`；发现过期就 `ariadne-suggest-refresh`，review 后再 `ariadne-apply-suggestion`。
...
| `ariadne-check-pack`       | 只读检查 `.codebase/` 结构完整性      |
...
- `check` 只读，不写文件。
- 除首次 `ariadne-init` 外，所有变更都先 `suggest` 生成可 review 的建议，再由用户显式 `apply`。
